### Construct plot 3 for course project 1 in Coursera course: Exploratory Data Analysis
...
plot3 <- function() {
  ...
  data <- data[, datetime:=as.POSIXct(paste(Date, Time, sep=' '), 
                                      format='%d/%m/%Y %H:%M:%S')]
  ...
  legend('topright', 
         ...
         lty=c(1,1,1) ### lty corresponds to line widths
         )
  dev.off()
}